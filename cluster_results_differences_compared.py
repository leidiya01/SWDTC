...
cluster_labels_7D = np.array([np.where(sorted_indices == i)[0][0] for i in kmeans.labels_])

# 将七维聚类结果添加到数据框中
data['Cluster_Original_7D'] = cluster_labels_7D

# 初始化总平方和结果
total_squared_differences = []

# 逐一剔除一列，进行六维聚类并计算差异平方和
for i in range(len(columns)):
    ...
    # 计算差异的平方和
    squared_difference = np.sum((cluster_labels_7D - cluster_labels_subset) ** 2)
    total_squared_differences.append(squared_difference)

# 将总平方和结果加入数据框
for i, diff in enumerate(total_squared_differences):
    diff_col_name = f'Total_Squared_Diff_Drop_{columns[i]}'
    data[diff_col_name] = [diff] * len(data)  # 将总平方和填充到每行中

# 保存结果到新的Excel文件
output_path = 'clustered_data_with_differences.xlsx'
